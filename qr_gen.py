...
def make_qr_card(
    ...
    canvas.paste(
        ...
        (
            QR_BOX_X1 + (box_w - qr_size) // 2,
            QR_BOX_Y1 + (box_h - qr_size) // 2,
        ),
    )
    ...
    if tree_number.strip():
        ...
        fit_and_draw(
            ...
            cx=QR_CX,
            cy=(NUM_Y1 + NUM_Y2) // 2,
            max_w=int(canvas.width * 0.55),
            max_h=int(zone_h * 0.70),
    ...
    if eng and hin:
        ...
        fit_and_draw(
            ...
            hin,
            cx=QR_CX,
            cy=(ENG_NAME_Y1 + ENG_NAME_Y2) // 2,
            max_w=int(canvas.width * 0.62),
            max_h=int(hin_h * 0.75),
        ...
        fit_and_draw(
            ...
            eng,
            cx=QR_CX,
            cy=(HINDI_NAME_Y1 + HINDI_NAME_Y2) // 2,
            max_w=int(canvas.width * 0.62),
            max_h=int(eng_h * 0.80),
...
def _read_rows(file_bytes: bytes, filename: str) -> list[dict]:
    ...
    return list(
        csv.DictReader(
            io.StringIO(file_bytes.decode("utf-8-sig"))
        )
    )
